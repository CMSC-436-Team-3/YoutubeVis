...
def charthome2():
    ...
    sortby= ['weapon','district','video','weekday','month','hour','age', 'gender', 'ethnicity', 'race']
    video_set, weapon_set, district_set = ['All'], ['All'], ['All']
    for video in Video.query.all():
        hold = video.description
        hold = hold.replace(' ','_')
        hold1 = video.weapon
    ...
    weapon_set.append('Not_Recorded'), video_set.append('Not_Recorded'), district_set.append('Not_Recorded')
    ...
    for video in filtered_list:
        if (video.latitude != '0' and video.latitude != ''): latitudes.append(video.latitude)
        if (video.longitude != '0' and video.longitude != ''): longitudes.append(video.longitude)

    print(len(latitudes))
    print(len(longitudes))

    return render_template('chart_home2.html', districts = district_set, sortby = sortby,  videos = video_set, weapons = weapon_set, s_s = choose_s, s_w = s_w, s_d = s_d, s_c = s_c, young = min_t_str, old = max_t_str, lats = latitudes, longs = longitudes)
...
def ret_sort(video, sort):
    if sort == 'weapon':
        ...
        return video.ethnicity
    elif sort == 'race':
        return video.race
    elif sort == 'district':
        return video.district
    elif sort == 'weekday':
        time = video.datetime
...
def get_filtered_list():
    ...
    for i in range(len(video_set)):
        ...
        if video_set[i] == 'Not_Recorded':
            video_set[i] = ''
    for video in Video.query.all():
        d, w, c = video.district, video.weapon, video.description
        d, w, c = d.replace('_',' '), w.replace('_',' '), c.replace('_',' ')
        timedate = video.datetime
